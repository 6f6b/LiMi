...
@interface QPSSZipArchive ()
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime;
@end


@implementation QPSSZipArchive {
	NSString *_path;
	NSString *_filename;
    zipFile _zip;
}
...
+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray *)paths {
	BOOL success = NO;
	QPSSZipArchive *zipArchive = [[QPSSZipArchive alloc] initWithPath:path];
	if ([zipArchive open]) {
...
//    NSLog(@"解压前 == %@ ==== %@",pItem.resourceUrl,pItem.locaIconUrl);
//    return [self unzipPasterItem:pItem AtPath:path toDestination:destination overwrite:YES password:nil error:nil delegate:delegate];
//}
...
//    // Message delegate
//    if (success) {
//        block(YES,pItem,path,destination);
//    }
//
//}

//+ (BOOL)unzipPasterItem:(PasterItem *)pItem AtPath:(NSString *)path toDestination:(NSString *)destination overwrite:(BOOL)overwrite password:(NSString *)password error:(NSError **)error delegate:(id<QPSSZipArchiveDelegate>)delegate {
//    // Begin opening
//    zipFile zip = unzOpen((const char*)[path UTF8String]);
//    if (zip == NULL) {
//        NSDictionary *userInfo = [NSDictionary dictionaryWithObject:@"failed to open zip file" forKey:NSLocalizedDescriptionKey];
...
//    
//    // Message delegate
//    if (success && [delegate respondsToSelector:@selector(zipArchiveDidUnzipPasterItem:AtPath:zipInfo:unzippedPath:)]) {
//        [delegate zipArchiveDidUnzipPasterItem:pItem AtPath:path zipInfo:globalInfo unzippedPath:destination];
//    }
//    return success;
//}
@end
