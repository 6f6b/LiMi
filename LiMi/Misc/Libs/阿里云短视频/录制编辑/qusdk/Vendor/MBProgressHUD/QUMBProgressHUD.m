...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"QUMBProgressHUD needs to be accessed on the main thread.");

CGFloat const QUMBProgressMaxOffset = 1000000.f;
...
@interface QUMBProgressHUDRoundedButton : UIButton
@end


@implementation QUMBProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    QUMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    QUMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (QUMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (QUMBProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = QUMBProgressHUDAnimationFade;
    _mode = QUMBProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(QUMBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == QUMBProgressHUDAnimationZoom) {
        type = animatingIn ? QUMBProgressHUDAnimationZoomIn : QUMBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == QUMBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == QUMBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == QUMBProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == QUMBProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    QUMBProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<QUMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    QUMBBackgroundView *backgroundView = [[QUMBBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = QUMBProgressHUDBackgroundStyleSolidColor;
    ...
    QUMBBackgroundView *bezelView = [QUMBBackgroundView new];
    ...
    UIButton *button = [QUMBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[QUMBRoundProgressView class]];

    QUMBProgressHUDMode mode = self.mode;
    if (mode == QUMBProgressHUDModeIndeterminate) {
    ...
    else if (mode == QUMBProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[QUMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == QUMBProgressHUDModeDeterminate || mode == QUMBProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[QUMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == QUMBProgressHUDModeAnnularDeterminate) {
            [(QUMBRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == QUMBProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == QUMBProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[QUMBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[QUMBProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[QUMBRoundProgressView class]]) {
        QUMBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [QUMBRoundProgressView appearanceWhenContainedIn:[QUMBProgressHUD class], nil];
#else
        appearance = [QUMBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[QUMBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((QUMBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((QUMBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[QUMBBarProgressView class]]) {
        QUMBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [QUMBBarProgressView appearanceWhenContainedIn:[QUMBProgressHUD class], nil];
#else
        appearance = [QUMBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[QUMBProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((QUMBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((QUMBBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    QUMBBackgroundView *bezelView = self.bezelView;
...
- (void)setMode:(QUMBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == QUMBProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation QUMBRoundProgressView
...
@implementation QUMBBarProgressView
...
@implementation QUMBBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = QUMBProgressHUDBackgroundStyleBlur;
            ...
            _style = QUMBProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(QUMBProgressHUDBackgroundStyle)style {
    if (style == QUMBProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = QUMBProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    QUMBProgressHUDBackgroundStyle style = self.style;
    if (style == QUMBProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == QUMBProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation QUMBProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [QUMBProgressHUD allHUDsForView:view];
    for (QUMBProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable QUMBProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    QUMBBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == QUMBProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    MBMainThreadAssert();
    self.backgroundView.style = QUMBProgressHUDBackgroundStyleSolidColor;
...
@implementation QUMBProgressHUDRoundedButton
