...
#define kSTTextviewDefaultHeight 40
#define kSTTextviewMaxHeight 80

@interface STInputBar () <UITextViewDelegate>

@property (strong,nonatomic) UIView *inputContainView;
...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, kSTIBDefaultHeight)]){
        ...
        _switchKeyboardImages = @[@"pl_icon_nor",@"pl_icon_pre"];
        [self loadUI];
    }
    return self;
}
...
- (void)loadUI{
    ...
    self.inputContainView = [UIView new];
    self.inputContainView.frame = CGRectMake(kSTLeftButtonWidth, (kSTIBDefaultHeight-40)/2, [UIScreen mainScreen].bounds.size.width-kSTLeftButtonWidth-25,40);
    self.inputContainView.backgroundColor = [UIColor colorWithRed:228 green:228 blue:228 alpha:1];
    ...
    [self.sendButton setImage:[UIImage imageNamed:@"pl_btn_send"] forState:UIControlStateNormal];
...
//    self.sendButton.enabled = NO;
    ...
    self.separateLine.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:1];
    self.separateLine.frame = CGRectMake(0, 0, self.frame.size.width, 1);
    
    [self addSubview:_keyboardTypeButton];
    [self addSubview:self.inputContainView];
